...
rep('<style>\n:root{', '<link rel="stylesheet" href="css/finder.css">\n<style>\n:root{')
...
def swap_shot(id_, file_):
    ...
    assert m, id_
    h = h[:m.start(1)] + m.group(1) + "img/" + file_ + h[m.end(2) - 1:]
...
rep("      else return {bg:document.getElementById(k[1]==='house'?'shotHouse':'shotBoats').style.backgroundImage};",
    "      else return {bg:'url(img/'+(k[1]==='house'?'hero':'riverboats')+'.jpg)'};")
...
rep("  document.getElementById('find2Shot').style.backgroundImage=shotHouse.style.backgroundImage;",
    "  document.getElementById('find2Shot').style.backgroundImage='url(img/hero.jpg)';")
