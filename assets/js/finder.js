...
(function () {
  ...
  var AVAILABILITY_API = window.RR_AVAILABILITY_API || "/api/availability";
  ...
  function fmtStamp(iso) {
    var d = new Date(iso);
    return isNaN(d) ? "earlier" : d.toLocaleDateString("en-GB", { day: "numeric", month: "short" }) + " at " + d.toLocaleTimeString("en-GB", { hour: "2-digit", minute: "2-digit" });
  }
  ...
  function renderResults() {
    ...
    if (state.checking) {
      ...
      lede = (n + r.rest.length) + " of the " + a.count + " cottages free from " + when + " fit what you've told us." + (r.taken.length ? " Another " + r.taken.length + " would fit but " + (r.taken.length === 1 ? "is" : "are") + " already booked." : "") + " Book straight through to the cottage." + (a.stale ? " Availability last checked " + fmtStamp(a.checked_at) + "." : "");
    } else {
